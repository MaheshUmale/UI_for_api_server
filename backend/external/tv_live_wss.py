...
class TradingViewWSS:
    ...
    def ensure_chart_session(self, symbol, interval):
        ...
        from backend.config import TV_STUDY_ID
        if TV_STUDY_ID:
            try:
    ...
    def on_error(self, ws, error):
        # Silence standard goodbye/close frames
        if "goodbye" in str(error).lower() or "1000" in str(error):
            logger.debug(f"TV WSS non-critical error: {error}")
        else:
    ...
    def on_close(self, ws, close_status_code, close_msg):
        if close_status_code == 1000 or (not close_status_code and "goodbye" in str(close_msg).lower()):
            logger.info("TV WSS Closed Normally")
        else:
            logger.warning(f"TV WSS Closed: {close_status_code} {close_msg}")
